...
export function idempotency(options) {
    ...
    return async (req, res, next) => {
        ...
        if (!key)
            return next(createHttpError(400, 'missing_idempotency_key'));
        if (!req.user?.sub)
        ...
        if (existing) {
            if (existing.requestHash !== requestHash)
                return next(createHttpError(409, 'idempotency_key_reused'));
        ...
        res.json = ((body) => {
            void IdempotencyKeyModel.updateOne({ userId, key, method, path }, { $set: { state: 'completed', responseStatus: res.statusCode, responseBody: body } }).exec();
